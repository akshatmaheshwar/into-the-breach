...
class Mech(Entity):
    ...
    def get_old_position(self) -> tuple[int, int]:
        """
        (tuple[int, int]) Returns the position of the mech just before it last 
        changed position, or the current position of the mech if it has never 
        moved.
        """
...
class Enemy(Entity):
    ...
    def update_objective(self, entities: list[Entity],
        ...
        """
        Updates the enemy's objective to a position decided upon based on a set
        of entities and buildings from the game. 
...
class BreachModel():
    ...
    def attempt_move(self, entity: Entity, position: tuple[int, int]) -> None:
        ...
        if (
            ...
            # Record movement
            self._prev_moves.append(entity)

    def undo_move(self) -> None:
        """
        Undoes the move made most recently, provided it was made after end_turn 
        was last called
        """
        # Nothing to undo
        if len(self._prev_moves) == 0:
            return

        last_entity = self._prev_moves.pop()
        last_entity.set_position(last_entity.get_old_position())
        last_entity.enable()
    ...
    def __init__(
        ...
        Args:
            ...
            load_callback (Optional[Callable[[], None]]): Callback that should 
                                                          ...
                                                          user clicks the 
                                                          "Load Game" button
            undo_callback (Optional[Callable[[], None]]): Callback that should 
                                                          be called when the 
                                                          user clicks the 
                                                          "Undo Move" button
...
class ControlBar(tk.Frame):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self._undo_button = tk.Button(self, 
                                      text=UNDO_TEXT, command=undo_callback)
        ...
        self._undo_button.pack(side=tk.LEFT, expand=tk.TRUE)
...
class IntoTheBreach:
    ...
    def _undo_move(self) -> None:
        """
        Undoes the most recent move, provided it was made after the user 
        last clicked the "End Turn" button.
        """
        self.set_focussed_entity(None)
